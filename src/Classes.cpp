...
Vector rotate(const Vector& a, const double& theta) {
	double comp1 = dot(a,Vector(cos(theta) , 0, sin(theta)));
	double comp2 = dot(a,Vector(0           , 1, 0         ));
	double comp3 = dot(a,Vector(-sin(theta), 0, cos(theta)));
...
void Geometry::readOBJ(const char* obj){//, bool load_textures) {
		...
		FILE* f;
		f = fopen(obj, "r");//if(!f){std::cout <<1 << std::endl;}
		int curGroup = -1;
		while (!feof(f)) {
			char line[255];
